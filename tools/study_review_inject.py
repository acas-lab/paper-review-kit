...
정본: samples/cares (모체는 papers/26. cares, 2026-07-09).
"""
import sys
...
SR_CSS = """
/* ==== Paper Study 검토 피드백 (study_review_inject.py) ==== */
...
SR_JS = r"""
<script>
/* ==== Paper Study 검토 피드백 (study_review_inject.py) — 브릿지 /review-study 연동 ==== */
(function(){
  var SHORT = "__SHORT__";
  ...
  (function attach(){
    ...
    btn.addEventListener('click', function(){
      ...
      window.__qaBridgeEnsure(function(up){
        ...
        setStatus('<span class="sr-spin"></span>답변을 기준 분석과 대조 중… (수십 초~몇 분)');
        fetchT(bridgeUrl()+'/review-study', {method:'POST', headers:{'Content-Type':'text/plain;charset=UTF-8'}, body: JSON.stringify({short: SHORT, answers: answers})}, 600000)
          ...
          .then(function(res){
            ...
            } else if(res.status === 'empty'){ setStatus(res.message || '작성된 답변이 없습니다.', 'err'); bodyEl.innerHTML=''; }
            else if(res.status === 'busy'){ setStatus(res.message || '이미 처리 중입니다.', 'err'); }
            else { setStatus('⚠ ' + (res.message || '검토 실패'), 'err'); }
          })
          .catch(function(err){
            if(err && err.name === 'AbortError') setStatus('⚠ 응답이 너무 오래 걸려 중단됐습니다. 브릿지 콘솔 로그를 확인하세요.', 'err');
            else setStatus('⚠ 검토 요청 중 오류가 발생했습니다. 브릿지 콘솔 로그를 확인하세요.', 'err');
          })
